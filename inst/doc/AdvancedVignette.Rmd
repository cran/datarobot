...
Once the modeling process has completed, the `GetAllModels` function returns an S3 object of class `listOfModels` that characterizes all of the models in a specified DataRobot project. It is important to use `WaitforAutopilot` before calling `GetAllModels`, as the function will return only a partial list (and a warning) if the autopilot is not yet complete.

...
`dataSource` defines the data used for predicitons, `projectName` defines the name of the project, `target` defines what to predict, and `wait = TRUE` tells the function to wait until all modeling is complete before executing other computations.
...
The worst model is a trivial "interecept-only" model that assigns the mean `medv` value to every record as its predicted response. This model is included in the project as a benchmark against which to compare the other, more useful prediction models.

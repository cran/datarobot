...
#  datetime_partition  <- CreateDatetimePartitionSpecification(datetimePartitionColumn = 'earliest_cr_line', numberOfBacktests = 5)
#  partitioning_method <- GenerateDatetimePartition(project = proj, spec = datetime_partition)
#  SetTarget(project = proj, target = "is_bad", mode = "quick", partition = partitioning_method)

## ----backtest_specification_example, echo = TRUE, eval = FALSE-----------
#  backtest <- list()
#  # Dates are not project specific but rather example dates
#  backtest[[1]] <- CreateBacktestSpecification(0, ConstructDurationString(), "1989-12-01", ConstructDurationString(days=100))
#  backtest[[2]] <- CreateBacktestSpecification(1, ConstructDurationString(), "1999-10-01", ConstructDurationString(days=100))
#  # create desired partition specification
#  partition <- CreateDatetimePartitionSpecification('earliest_cr_line', numberOfBacktests = 2, backtests=backtest)
...
#  modelJobId_frozen   <- RequestFrozenDatetimeModel(dt_model,  trainingStartDate = as.Date('1950/12/1'), trainingEndDate = as.Date('1998/3/1'))
...
#  modelJobId   <- RequestNewDatetimeModel(proj, bps[[1]],  trainingDuration = ConstructDurationString(months=10))
#  new_dt_model <- GetDatetimeModelFromJobId(proj, modelJobId)
